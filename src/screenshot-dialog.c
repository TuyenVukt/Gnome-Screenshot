...
button_clicked (GtkWidget *button, ScreenshotDialog *dialog)
{
  ...
  else if (button == dialog->back_button)
      res = SCREENSHOT_RESPONSE_BACK;
  else {
        res = SCREENSHOT_RESPONSE_EDIT;
  }
                       ...
                       gpointer               user_data)
{
  ...
  // 
  dialog->edit_button = GTK_WIDGET (gtk_builder_get_object (ui, "edit_button"));
  g_signal_connect (dialog->edit_button, "clicked", G_CALLBACK (button_clicked), dialog);
